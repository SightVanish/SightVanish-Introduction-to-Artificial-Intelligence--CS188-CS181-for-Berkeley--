...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        p_CPT = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # join->eliminate->normalize in order
        for i in eliminationOrder:
            join_CPT = joinFactorsByVariable(p_CPT, i)
            p_CPT = join_CPT[0]
            # note that if there is only 1 unconditionedVariables, it can not be eliminated
            if len(join_CPT[1].unconditionedVariables()) != 1:
                p_CPT.append(eliminate(join_CPT[1], i))
        # note! join must be the last step        
        return normalize(joinFactors(p_CPT))
