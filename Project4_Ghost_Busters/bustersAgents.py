        ...
        for ghost in livingGhostPositionDistributions:
            ...
            for pos in ghost:
                if ghost[pos] > v:
                    v = ghost[pos]
                    maxpos = pos
            d = self.distancer.getDistance(pacmanPosition, maxpos)
            if d < dist:
                dist = d
                near_ghost = maxpos

        # choose action that minimize the distance
        result = None
        new_dist = float('inf')
        # test all legal movement
        for action in legal:
            new_pos = Actions.getSuccessor(pacmanPosition, action)
            d = self.distancer.getDistance(new_pos, near_ghost)
            # find the nearest new pos(caused by action)
            if d < new_dist:
                new_dist = d
                result = action
        return result
